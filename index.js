...
export const handler = async (event, context) => {
  const corsOptions = {
    ...
    optionsFailedStatus: 204,
    allowedHeaders: ["Content-Type", "Authorization", "x-forwarded-proto"],
  };

  const isPreflight = event.httpMethod === "OPTIONS";

  if (isPreflight) {
    console.log("CORS Options for Preflight:", corsOptions);
    const isSuccess = event.headers["Origin"] === corsOptions.origin;
    const status = isSuccess
      ? corsOptions.optionsSuccessStatus
      : corsOptions.optionsFailedStatus;

    // Log success or failure to the console
    console.log(
      `CORS Preflight Status: ${isSuccess ? "Success" : "Failed"} (${status})`
    );

    // Respond with the appropriate status code for the preflight request
    return {
      statusCode: status,
      headers: corsOptions,
    ...
    try {
      ...
      if (
        ...
        User.findOne({ email }).then((user) => {
          ...
          const role = determineRoleFromDesignation(
  ...
  app.get("/last-login", async (req, res) => {
    try {
      const lastLogins = await LastLogin.find().populate(
        "userId",
        "name email"
      ); // Assuming you have a reference to the user in LastLogin model
      ...
      const result = await Analyst.aggregate([
        ...
        },
        {
          $group: {
            _id: {
              date: {
                $dateToString: { format: "%Y-%m-%d", date: "$dateTask" },
              },
